...
def get_auth_url(credential_id: int, source: DocumentSource) -> str:
    ...
    get_kv_store().store(
        KV_CRED_KEY.format(credential_id), params.get("state", [None])[0]
    )  # type: ignore
    return str(auth_url)
...
def upsert_google_app_cred(
    ...
    if source == DocumentSource.GOOGLE_DRIVE:
        get_kv_store().store(
            KV_GOOGLE_DRIVE_CRED_KEY, app_credentials.json()
        )
    elif source == DocumentSource.GMAIL:
        get_kv_store().store(KV_GMAIL_CRED_KEY, app_credentials.json())
    else:
        raise ValueError(f"Unsupported source: {source}")
...
def upsert_service_account_key(
    ...
    if source == DocumentSource.GOOGLE_DRIVE:
        ...
        get_kv_store().store(
            KV_GMAIL_SERVICE_ACCOUNT_KEY, service_account_key.json()
        )
    else:
        raise ValueError(f"Unsupported source: {source}")
