...
from backend.auth.api_key import build_displayable_api_key
from backend.auth.api_key import generate_api_key
from backend.auth.api_key import hash_api_key
from backend.auth.schemas import UserRole
from backend.db.models import ApiKey
from backend.db.models import User


BACKEND_API_KEY_DUMMY_EMAIL_DOMAIN = "@api-key.backend.local"
BACKEND_API_KEY_PREFIX = "api-"
UNNAMED_KEY_PLACEHOLDER = "unnamed"


def get_api_key_email_pattern() -> str:
    return BACKEND_API_KEY_DUMMY_EMAIL_DOMAIN
...
def fetch_api_keys(db_session: Session) -> list[ApiKeyDescriptor]:
    ...
    return [
        ApiKeyDescriptor(
            ...
            api_key_role=UserRole(api_key.user.role),
...
def get_api_key_fake_email(
    ...
    return f"{BACKEND_API_KEY_PREFIX}{name}@{unique_id}{BACKEND_API_KEY_DUMMY_EMAIL_DOMAIN}"


def insert_api_key(
    db_session: Session, api_key_args: dict, user_id: uuid.UUID | None
) -> ApiKeyDescriptor:
    api_key = generate_api_key()
    api_key_user_id = uuid.uuid4()

    display_name = api_key_args.get("name") or UNNAMED_KEY_PLACEHOLDER
    # Generate a random password for the API key "user"
    random_password = secrets.token_urlsafe(32)
    
    api_key_user_row = User(
        id=api_key_user_id,
        email=get_api_key_fake_email(display_name, str(api_key_user_id)),
        # a random password for the "user" - just hashed with simple approach
        hashed_pw=f"api_key_user_{random_password}",
        role=api_key_args.get("role", UserRole.BASIC.value),
        organization_id=api_key_args.get("organization_id"),
    )
    db_session.add(api_key_user_row)

    api_key_row = ApiKey(
        name=api_key_args.get("name"),
    ...
    return ApiKeyDescriptor(
        ...
        api_key_role=UserRole(api_key_user_row.role),
        api_key_display=api_key_row.api_key_display,
        api_key=api_key,
        api_key_name=api_key_args.get("name"),
        user_id=api_key_user_id,
    )


def update_api_key(
    db_session: Session, api_key_id: int, api_key_args: dict
) -> ApiKeyDescriptor:
    ...
    existing_api_key.name = api_key_args.get("name")
    ...
    email_name = api_key_args.get("name") or UNNAMED_KEY_PLACEHOLDER
    api_key_user.email = get_api_key_fake_email(email_name, str(api_key_user.id))
    api_key_user.role = api_key_args.get("role", api_key_user.role)
    db_session.commit()
    ...
    return ApiKeyDescriptor(
        ...
        api_key_name=api_key_args.get("name"),
        api_key_role=UserRole(api_key_user.role),
