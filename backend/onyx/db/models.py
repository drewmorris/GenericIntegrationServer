...
class ConnectorCredentialPair(Base):
    ...
    # MULTI-TENANT INTEGRATION SERVER EXTENSIONS
    # Added by Phase 1: Database Foundation
    organization_id: Mapped[UUID | None] = mapped_column(
        PGUUID(as_uuid=True), ForeignKey("user.id"), nullable=True, index=True
    )
    destination_target_id: Mapped[UUID | None] = mapped_column(
        PGUUID(as_uuid=True), ForeignKey("destinationtarget.id"), nullable=True, index=True
    )
...
class DestinationTarget(Base):
    """Destination targets for multi-tenant document routing"""
    __tablename__ = "destinationtarget"
    
    id: Mapped[UUID] = mapped_column(PGUUID(as_uuid=True), primary_key=True, default=uuid4)
    organization_id: Mapped[UUID] = mapped_column(PGUUID(as_uuid=True), ForeignKey("user.id"), nullable=False)  # Using User as org for simplicity
