    ...
    useEffect(() => {
        const handleOAuthCallback = async () => {
            try {
                ...
                if (response.data) {
                    setStatus('success');
                    setMessage(`${provider === 'gmail' ? 'Gmail' : 'Google Drive'} OAuth completed successfully!`);

                    enqueueSnackbar(
                        `${provider === 'gmail' ? 'Gmail' : 'Google Drive'} credential created: ${response.data.email}`,
                        { variant: 'success' }
                    );

                    // Redirect to connectors page with credential info
                    setTimeout(() => {
                        navigate(`/connectors?credential_id=${response.data.credential_id}&connector=${provider === 'gmail' ? 'gmail' : 'google_drive'}`);
                    }, 2000);
                }
            } catch (error: any) {
